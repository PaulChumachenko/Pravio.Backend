...
class YumUser extends YumActiveRecord
{
	...
	public function getPermissions()
	{
		...
		$permissions = array();
		foreach ($roles as $role) {
			$sql = "select id, ".Yii::app()->db->tablePrefix."action.title from ".Yii::app()->db->tablePrefix."permission left join ".Yii::app()->db->tablePrefix."action on ".Yii::app()->db->tablePrefix."action.id = ".Yii::app()->db->tablePrefix."permission.action where type = 'role' and principal_id = {$role->id}";
		...
		$sql = "select id, ".Yii::app()->db->tablePrefix."action.title from ".Yii::app()->db->tablePrefix."permission left join ".Yii::app()->db->tablePrefix."action on ".Yii::app()->db->tablePrefix."action.id = ".Yii::app()->db->tablePrefix."permission.action where type = 'user' and principal_id = {$this->id}";
		foreach (Yii::app()->db->cache(500)->createCommand($sql)->query()->readAll() as $permission)
			$permissions[$permission['id']] = $permission['title'];
				...
				/*
							...
							$options));
                                ...
                                if( $this->profile->sex == 0 )
                                    $return .= CHtml::image(Yii::app()->getAssetManager()->publish(
                                                            Yii::getPathOfAlias('YumAssets.images') . ($thumb
                                                                    ? '/no_avatar_available_thumb_male.jpg' : '/no_avatar_available_male.jpg'),
                                                            Yum::t('No image available'),
                                                            $options));
                                else
                                    $return .= CHtml::image(Yii::app()->getAssetManager()->publish(
							Yii::getPathOfAlias('YumAssets.images') . ($thumb
								? '/no_avatar_available_thumb_female.jpg' : '/no_avatar_available_female.jpg'),
							Yum::t('No image available'),
							$options));
			$return .= '</div><!-- avatar -->';			
                        return $return;                        
