	...
	public function actionRemoveAvatar($id = null) {		
		...
		if(!$model)
			throw new CHttpException(404);
                ...
                if (is_readable(realpath($model->avatar)) && ($model->avatar != 'gravatar') && ($model->avatar != '') ) unlink(realpath($model->avatar)); 
		...
		if($id && Yii::app()->user->isAdmin())
                {
                        if ($id != NULL) $this->redirect(array('//profile/profile/view&id='.$id));
                        else $this->redirect(array(Yum::module('profile')->profileViewRoute));
                        Yii:app()->end;
                }                    
                else                       
                        $this->redirect(array(Yum::module('profile')->profileViewRoute));	
	}
